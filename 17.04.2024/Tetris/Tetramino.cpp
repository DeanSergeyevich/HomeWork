...
Tetramino::Tetramino(Texture& texture, int correct_x, int correct_y) : m_type{ (mt19937(random_device{}())()%7)},
m_tetratype
{
	1,3,5,7, // I
	2,4,5,7, // S
...
{
	Color color = m_color.at(mt19937(random_device{}())() % 4);
...
void Tetramino::setSpritesCords(array<Point, 4>& arr)
{
	m_sprites_cords = arr;
	for(int i = 0; i < m_sprites.size(); i++)
	{
...
{
	m_sprites.at(sprite_index).setPosition(m_correction_x + m_sprites_cords.at(sprite_index).x * 18.f,
		m_correction_y + m_sprites_cords.at(sprite_index).y * 18.f);
}